...
__ztl_namespace_start
	struct trie_node  {
		...
		trie_node() : m_value(NULL), m_end(false), m_nodes(map<char, trie_node*, __stl::less<char>>()) { }
		trie_node(char _v) : m_value(_v), m_end(false), m_nodes(map<char, trie_node*, __stl::less<char>>()) {	}

		trie_node* operator[](char c) { return m_nodes.find(c) == m_nodes.end() ? NULL : m_nodes[c]; }
	...
	public:
		...
		inline void insert(vector<__stl::string> sv) { 
